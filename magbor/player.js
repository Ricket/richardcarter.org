...
function Player() {
	...
	Player.prototype.LoadImage = function (imageSrc) {
		...
		var tempBlah = this;
		this.image.onmouseover = function () { tempBlah.ShowNameTag(); };
		this.image.onmouseout = function () { tempBlah.HideNameTag(); };
	}
	...
	Player.prototype.ShowNameTag = function () {
		...
		if(parseInt(playerName.style.left)+parseInt(playerName.style.width)>mapWidth*32)
			playerName.style.left = (mapWidth - parseInt(playerName.style.width))+"px";
	...
	Player.prototype.WalkBy = function (xOffset,yOffset) {
		...
		var tmp = this;
		setTimeout(function(){tmp.UpdateMove();},100);
	}
