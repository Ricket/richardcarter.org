...
function Class(id, cn, room, from, to, mon,tues,wed,thurs,fri) {
	this.id = id;
	this.courseName = cn;
	this.room = room;
	this.from = from;
	...
	this.intersectsWith = function (otherClass) {
		if( (!this.mon   || !otherClass.mon  ) && 
		    ...
		    (!this.fri   || !otherClass.fri) )
		{
			...
			if(this.to.isLessThan(otherClass.from) ||
			   otherClass.to.isLessThan(this.from))
			{
				return false;
			} else {
				return true;
...
function addClass() {
	var from = timeparse(document.forms[0].elements[frm0elems['from']].value);
	if(from == null) {
	...
	var to = timeparse(document.forms[0].elements[frm0elems['to']].value);
	if(to == null) {
	...
	for(var i=frm0elems['m'];i<=frm0elems['f'];i++) {
	...
	var newclass = new Class("row_"+rownum, document.forms[0].elements[frm0elems['name']].value, document.forms[0].elements[frm0elems['room']].value, from, to,
	document.forms[0].elements[frm0elems['m']].checked,document.forms[0].elements[frm0elems['t']].checked,
	document.forms[0].elements[frm0elems['w']].checked,document.forms[0].elements[frm0elems['h']].checked,document.forms[0].elements[frm0elems['f']].checked);
	for(var i=0;i<classes.length;i++) {
		if(newclass.intersectsWith(classes[i])) {
			alert("class overlaps an existing class");
			return;
		}
	}
	...
	nametd.appendChild(document.createTextNode(document.forms[0].elements[frm0elems['name']].value));
	row.appendChild(nametd);
	
	var nametd = document.createElement("td");
	nametd.appendChild(document.createTextNode(document.forms[0].elements[frm0elems['room']].value));
	row.appendChild(nametd);
	...
	var days = "";
	if(document.forms[0].elements[frm0elems['m']].checked)
		days += "M";
	if(document.forms[0].elements[frm0elems['t']].checked)
		days += "T";
	if(document.forms[0].elements[frm0elems['w']].checked)
		days += "W";
	if(document.forms[0].elements[frm0elems['h']].checked)
		days += "H";
	if(document.forms[0].elements[frm0elems['f']].checked)
		days += "F";
